...
typedef enum _PLUGIN_TYPE {
	...
	PLT_EXPLORERBAR,							// プラグインバーに格納されるエクスプローラバープラグイン
	PLT_STATUSBAR,								// ステータスバーに格納されるステータスバープラグイン(未実装です)
	PLT_OPERATION,								// 表には表示されないバックグラウンドプラグイン(未実装です)
	PLT_DOCKINGBAR, 							// ドッキングウィンドウとして配置されるドッキングバープラグイン
} PLUGIN_TYPE;
