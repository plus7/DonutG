                                           ...
                                           nsILocalFile **_retval)
{
    ...
    TCHAR szPath[MAX_PATH + 1];
    memset(szPath, 0, sizeof(szPath));
    wcsncpy(szPath, aDefaultFile, MAX_PATH);
    ...
    ofn.lpstrDefExt = aSuggestedFileExtension;
    ofn.lpstrFile = szPath;
    ofn.nMaxFile = MAX_PATH;

    if (GetSaveFileName(&ofn))
    {
		nsEmbedString strPath(szPath);
        return NS_NewLocalFile(strPath, PR_FALSE, _retval);
    }
