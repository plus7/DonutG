...
static const TCHAR* kAsLaidOutOnScreenStr = _T("As &laid out on the screen");
static const TCHAR* kTheSelectedFrameStr  = _T("The selected &frame");
static const TCHAR* kEachFrameSeparately  = _T("&Each frame separately");
...
GetLocalizedString(nsIStringBundle* aStrBundle, const char* aKey, nsString& oVal)
{
  ...
  nsEmbedString valUni;
  nsEmbedString key(NS_ConvertASCIItoUTF16(aKey).get());
...
{
  HWND hWnd = ::CreateWindow (aType, aStr,
                           ...
                           const nsIntRect& aRect)
{
  ...
  return CreateControl(_T("BUTTON"), BS_RADIOBUTTON, aHInst, aHdlg, aId, aStr, aRect);
}
...
static UINT CALLBACK PrintHookProc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam) 
{
  ...
  if (uiMsg == WM_COMMAND) {
    ...
    HWND grpBoxWnd = CreateGroupBox(hInst, hdlg, grp3, _T("Print Frame"), rect);
    if (grpBoxWnd == NULL) {
      Show(rad4Wnd, FALSE); // hide
      Show(rad5Wnd, FALSE); // hide
...
static BOOL APIENTRY PropSheetCallBack(HWND hdlg, UINT uiMsg, UINT wParam, LONG lParam)
{
  if (uiMsg == WM_COMMAND) {
    ...
    HWND grpBoxWnd = CreateGroupBox(hInst, hdlg, grp3, NS_LITERAL_STRING("Print Frame").get(), rect);
    if (grpBoxWnd == NULL) {
      Show(rad4Wnd, FALSE); // hide
      Show(rad5Wnd, FALSE); // hide
      Show(rad6Wnd, FALSE); // hide
