...
NS_IMETHODIMP nsPrintProgressParams::GetDocTitle(PRUnichar * *aDocTitle)
{
  ...
  *aDocTitle = (PRUnichar*)NS_Alloc(sizeof(PRUnichar) * (mDocTitle.Length() + 1));
  wcscpy(*aDocTitle, mDocTitle.get());
  return NS_OK;
}
...
NS_IMETHODIMP nsPrintProgressParams::GetDocURL(PRUnichar * *aDocURL)
{
  ...
  *aDocURL = (PRUnichar*)NS_Alloc(sizeof(PRUnichar) * (mDocTitle.Length() + 1));
  wcscpy(*aDocURL, mDocURL.get());
  return NS_OK;
}
