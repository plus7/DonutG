                                                  ...
                                                  PRBool *notifyOnOpen)
{
  *notifyOnOpen = PR_TRUE;
  ...
  if (parent)
  {
    ...
    /*nsCOMPtr<nsISupportsArray> array = do_CreateInstance("@mozilla.org/supports-array;1", &rv);
...
NS_IMETHODIMP nsPrintProgress::CloseProgressDialog(PRBool forceClose)
{
	...
	return NS_OK; //OnStateChange(nsnull, nsnull, nsIWebProgressListener::STATE_STOP, forceClose);
}
...
NS_IMETHODIMP nsPrintProgress::RegisterListener(nsIWebProgressListener * listener)
{
  ...
  if (!m_listenerList)
  {
	  m_listenerList = do_CreateInstance("@mozilla.org/supports-array;1");
  }
