...
#define MSG_WM_USER_GET_IWEBBROWSER()	   \
	if (uMsg == WM_USER_GET_IWEBBROWSER) { \
		SetMsgHandled(TRUE);			   \
		lResult = (LRESULT) m_spBrowser.get(); \
		if ( IsMsgHandled() )			   \
			return TRUE;				   \
	}
...
#define MSG_WM_USER_GET_ACTIVE_IWEBBROWSER()				\
	if (uMsg == WM_USER_GET_IWEBBROWSER) {					\
		...
		if (hWnd != NULL)									\
			lResult = (LRESULT) DonutGetNsIWebBrowser(hWnd); \
		else												\
			lResult = NULL; 								\
		if ( IsMsgHandled() )								\
			return TRUE;									\
...
inline nsIWebBrowser *DonutGetNsIWebBrowser(HWND hWnd)
{
	return (nsIWebBrowser *) ::SendMessage(hWnd, WM_USER_GET_IWEBBROWSER, 0, 0);
}
