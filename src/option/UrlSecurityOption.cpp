...
bool CUrlSecurityOption::FindUrl(const CString &strURL, DWORD* pExProp, DWORD* pExPropOpt, DWORD* pFlags)
{
  ...
  #if 1	//+++
	int 	n = 0;
	CUrlSecurityOption::COptUrlList::iterator 	it;
	for (it = CUrlSecurityOption::s_urlSecurity.begin();
		it != CUrlSecurityOption::s_urlSecurity.end();
		++it)
	{
		CString 	ptn = it->m_url;
	  #ifdef USE_REGEX	//+++ 正規表現を有効にする場合.
		if (it->m_flags & 1) {
			if (ptn[0] != _T('*')) {
				try {	//正規表現が不正だとエラーが投げられるので、無視するようにする
					basic_regex<TCHAR> 		re(LPCTSTR(it->m_url));
					match_results<LPCTSTR>	results;
					if ( regex_match(LPCTSTR(strURL), results, re) ) {
						rc = true;
						break;
	 				}
				} catch (...) {
...
BOOL CUrlSecurityPropertyPage::_DoDataExchange(BOOL bSaveAndValidate)	// get data from controls?
{
	if (!bSaveAndValidate) {											// set data of control
		...
		std::for_each( CUrlSecurityOption::s_urlSecurity.begin(), CUrlSecurityOption::s_urlSecurity.end(), _AddToListBox(m_listbox) );
...
void CUrlSecurityPropertyPage::DispExOptsFlag(unsigned exopts, unsigned exopts2, unsigned flags)
{
	...
	CButton(GetDlgItem(IDC_CHECK_IMAGE         )).SetCheck( (dlc & GECKO_DLCTL_IMAGES  ) != 0 );
	CButton(GetDlgItem(IDC_CHECK_FRAME         )).SetCheck( (dlc & GECKO_DLCTL_FRAMES  ) != 0 );
	CButton(GetDlgItem(IDC_CHECK_PLUGIN        )).SetCheck( (dlc & GECKO_DLCTL_PLUGINS ) != 0 );
	CButton(GetDlgItem(IDC_CHECK_SCRIPT        )).SetCheck( (dlc & GECKO_DLCTL_SCRIPTS ) != 0 );
	CButton(GetDlgItem(IDC_CHECK_REDIR         )).SetCheck( (dlc & GECKO_DLCTL_REDIR   ) != 0 );
	CButton(GetDlgItem(IDC_CHECK_DNS           )).SetCheck( (dlc & GECKO_DLCTL_DNS     ) != 0 );
