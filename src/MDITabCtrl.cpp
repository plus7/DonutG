...
void CMDITabCtrl::SetMenuOwner(HWND hWndOwner)
{
...
void CMDITabCtrl::SetMDITabLogFont(const LOGFONT &lf)
{
	CFontHandle font;
...
BOOL CMDITabCtrl::GetMDITabLogFont(LOGFONT &lf)
{
	if (m_font.m_hFont) {
		return m_font.GetLogFont(&lf) != 0;
	}
...
void CMDITabCtrl::SetMaxTabItemTextLength(int nLength)
{
	...
	for (UINT i = 0; i < nCount; ++i) {
		...
		UINT nCount2 = GetItemCount();	//+++ 不要のはずだが、テストで用意...
		ATLASSERT(nCount == nCount2);
		nCount = nCount2;
	}
}
...
{	// Overrides
	...
	if (nIndex != -1) {
		...
		if (m_dwExtendedStyle & MTB_EX_RIGHTCLICKCLOSE) {
			...
			MTLVERIFY( menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | dwRLFlag /* minit*/,
											point.x,
											point.y,
...
HRESULT CMDITabCtrl::OnGetTabCtrlDataObject(CSimpleArray<int> &arrIndex, IDataObject **ppDataObject)
{
	...
	if ( SUCCEEDED(hr) ) {
		for (int i = 0; i < arrIndex.GetSize(); ++i) {
			...
			ATLASSERT( ::IsWindow(hWnd) );
			CGeckoBrowser browser = (nsIWebBrowser *) ::SendMessage(hWnd, WM_USER_GET_IWEBBROWSER, 0, 0);
			CString 	 strName = MtlGetWindowText(hWnd);
			CString 	 strUrl  = browser.GetLocationURL();
...
void CMDITabCtrl::ReloadSkin(int nStyle)
{
	CString strTabBmp = _GetSkinDir() + _T("Tab.bmp");
