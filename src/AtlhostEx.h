...
// ctor/dtor
	CAxHostWindow()
	{
		...
		m_bUIActive = FALSE;
	  #if 1 //+++ ãåî≈(atl3ån)Ç…Ç†ÇÌÇπÇƒÇ›ÇÈ
	...
	BEGIN_MSG_MAP( CAxHostWindow )
...
#endif
		{
			...
			if (dwHitResult == HITRESULT_HIT)
			{
				...
				//x MESSAGE_HANDLER(WM_SETCURSOR, OnWindowlessMouseMessage) 		//+++ ãåî≈Ç…Ç†ÇÌÇπÇƒí«â¡...Ç‚Ç¡ÇœÇËçÌèú.
		...
		//+++ if(!m_bWindowless && m_bMessageReflect)			//+++ ãåî≈Ç…Ç†ÇÌÇπÇÈ
		...
		*ppUnk = NULL;
		...
		if (::IsWindow(hWnd))
		{
			...
			if (SUCCEEDED(hr) && bWasHTML && *ppUnk != NULL)
			{
			  ...
			  #if 0 //*+++ donut(ån)Ç≈ÇÕÇ±ÇÃèàóùÇ™Ç†ÇÈÇ∆about:blankÇ™ÇÁÇ›ÇÃÉoÉOÇ™î≠ê∂Ç∑ÇÈ.
				//	//* Å¶ BeforeNavigateìôÇÃåƒÇ—èoÇµÉ^ÉCÉ~ÉìÉOÇ™ïœÇÌÇËèàóùÇ™îjí].
				//	else
				//	{
