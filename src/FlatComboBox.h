...
#ifdef USE_DIET	//+++ ダイエットのとき
#define NO_FLATCOMBOBOX
#endif


class CFlatComboBox	: public CWindowImpl<CFlatComboBox, CComboBox>, public CTrackMouseLeave<CFlatComboBox> {
...
public:
	void SetDrawStyle(int nStyle)
	{
	  #ifndef NO_FLATCOMBOBOX
		m_nDrawStyle = nStyle;
	  #else		//+++ flatcomboboxの仕組みを捨てる場合
		BOOL	//+++ 無理やりメンバーをローカル変数扱いにする
	  #endif
		m_bExistManifest = IsExistManifestFile();		//+++
