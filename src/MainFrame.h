...
public:
	...
	int 		ApiNewWindow(const PRUnichar *url, BOOL bActive);
	void 		ApiShowPanelBar();
	long 		ApiGetTabState(int nIndex);
	nsIWebBrowser*	ApiGetPanelWebBrowserObject();
	nsIDOMWindow*	ApiGetPanelWindowObject();
	nsIDOMDocument*	ApiGetPanelDocumentObject();
	void 		ApiExecuteCommand(int nCommand);	//IAPI2 by minit
	void 		ApiGetSearchText(PRUnichar **text);
	void 		ApiSetSearchText(const PRUnichar *text);
	void 		ApiGetAddressText(PRUnichar **text);
	void 		ApiSetAddressText(const PRUnichar *text);
	...
	long 		ApiGetProfileInt(const PRUnichar *file, const PRUnichar *section, const PRUnichar* key, int nDefault);
...
//		MSG_WM_USER_CHANGE_CSS			( OnChangeCSS )
		...
		COMMAND_ID_HANDLER( ID_OPEN_EXE_DIR			 , OnOpenExeDir 	)		//+++ donutのフォルダを開ける
...
private:
	...
	HWND 		UserOpenFile(const CString& strFile, DWORD openFlag, int dlCtrlFlag=-1, int extendedStyleFlags=-1);	//+++ 引数追加.
	...
	HWND 		OpenUrlWithExProp(CString strUrl, DWORD dwOpenFlag, DWORD dwExProp, DWORD dwExProp2);	//+++ 引数追加
	HWND 		OpenUrlWithExProp(CString strUrl, DWORD dwOpenFlag, CString strIniFile, CString strSection = DONUT_SECTION);

	//既存タブにナビゲートしたのちに拡張設定を適用する
...
private:
	...
	enum {
		...
		_nPosCssMenu		   = 4, _nPosSubCssMenu 	= 13/*plus7: もとは14だった*/, _nPosEncodeMenu = 2, _nPosEncodeMenuSub	 = 17
	};
