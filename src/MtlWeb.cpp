...
void MtlOrganizeFavorite(HWND hWnd, bool bOldShell, const CString &strPath_)
{
	...
	if (Misc::getIEMejourVersion() >= 8 && _CheckOsVersion_VistaLater() == 0)
		bOldShell = true;
  #endif
...
int GetGlobalCookiePref()
{
	...
	PRInt32 val;
	pref->GetIntPref("network.cookie.cookieBehavior", &val);
	return val;
}
...
{
	nsCOMPtr<nsIPrefBranch> pref = do_GetService("@mozilla.org/preferences-service;1");
	pref->SetIntPref("network.cookie.cookieBehavior", val);
}

int	GetSiteCookiePref(const CString& strURI)
{
	nsCOMPtr<nsIPermissionManager> perm = do_GetService("@mozilla.org/permissionmanager;1");
	nsCOMPtr<nsIURI> uri;
	NS_NewURI(getter_AddRefs(uri), nsEmbedString(strURI));
	PRUint32 ret = 0;
	perm->TestPermission(uri, "cookie", &ret);
	return ret;
}

void SetSiteCookiePref(const CString& strURI, int val)
{
	nsCOMPtr<nsIPermissionManager> perm = do_GetService("@mozilla.org/permissionmanager;1");
	nsCOMPtr<nsIURI> uri;
	NS_NewURI(getter_AddRefs(uri), nsEmbedString(strURI));
	perm->Add(uri, "cookie", val);
}
//TODO: なあにこれ
